...
agentkitche.dev supports multiple operating profiles so teams can start locally and grow into multi-machine or cloud deployments without rewriting code.
...
agentkitche.dev reads that registration and advertises `mcp-server:gitnexus` through tool-attention. This keeps code graph indexing, stale-index checks, and repo intelligence in GitNexus while Kitchen stays the progressive discovery and operator-control layer.
