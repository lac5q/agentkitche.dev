...
<action>
Create `apps/memroos/src/app/api/tool-attention/similar/route.ts`:
...
<automated>
npm --prefix apps/memroos run test -- --reporter=verbose 2>&1 | grep -E "similar|PASS|FAIL" | head -20
</automated>

<acceptance_criteria>
- File `apps/memroos/src/app/api/tool-attention/similar/route.ts` exists
- `grep -n "getSimilarTaskRecommendations" apps/memroos/src/app/api/tool-attention/similar/route.ts` exits 0
- `grep -n "force-dynamic" apps/memroos/src/app/api/tool-attention/similar/route.ts` exits 0
</acceptance_criteria>

### Task B — Implement the skipped route tests

<read_first>
- apps/memroos/src/app/api/tool-attention/similar/__tests__/route.test.ts
...
<automated>
npm --prefix apps/memroos run test -- --reporter=verbose 2>&1 | grep -E "similar|contextMatch|PASS|FAIL" | head -20
</automated>
...
<acceptance_criteria>
- `grep -n "useSimilarTaskRecommendations" apps/memroos/src/lib/api-client.ts` exits 0
- `grep -n "SimilarTaskResponse" apps/memroos/src/lib/api-client.ts` exits 0
- Full suite green
</acceptance_criteria>
