...
Memroos now treats external context lanes as explicit product contracts.
...
- Added `apps/memroos/src/lib/context-sources.ts` for contract loading,
...
- Added `npm run eval:context-sources` and Memroos workspace wiring.
...
- `PATH="/opt/homebrew/opt/node@22/bin:$PATH" npm --prefix apps/memroos run test -- src/lib/__tests__/context-sources.test.ts src/app/api/context/__tests__/health-route.test.ts --run`
