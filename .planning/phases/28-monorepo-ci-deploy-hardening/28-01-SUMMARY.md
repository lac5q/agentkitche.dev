...
Completed. The monorepo now has a CI workflow that validates the Memroos app and Python service surface, and the merged v1.6 code is deployed through the production LaunchAgent.

## Shipped

- Added `.github/workflows/ci.yml`.
- CI covers Memroos dependency install, Vitest suite, production build, Knowledge MCP tests, Python compile smoke, and shell script syntax checks.
...
- `npm --prefix apps/memroos run test -- --run` passed: 42 files, 331 tests.
