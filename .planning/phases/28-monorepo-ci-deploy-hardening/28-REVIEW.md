...
files_reviewed_list:
  ...
  - apps/memroos/src/lib/node-keyword-map.ts
...
Reviewed the monorepo CI workflows, Next.js Memroos tool-attention UI/API path, React Flow updates, and the Python knowledge MCP progressive-disclosure surface. The main blockers are in secret/PII guard coverage and filesystem path disclosure from the tool-attention catalog.

Verification performed: `npm --prefix apps/memroos run test -- --run` passed, `npm run build` passed with a Turbopack tracing warning, `python3 -m py_compile services/memory/*.py services/knowledge-mcp/knowledge_system/*.py` passed, `bash -n start.sh services/memory/*.sh` passed. Local `python` is unavailable and local `python3` does not have `pytest`; CI installs pytest before running Python tests.
...
**Issue:** The guard only checks for a repository-root `.env.local`. In this monorepo the Memroos app lives under `apps/memroos`, so a committed `apps/memroos/.env.local` would bypass the workflow even though it is the app-local secret file Next.js commonly loads.
...
**File:** `apps/memroos/src/lib/tool-attention.ts:257`, `services/knowledge-mcp/knowledge_system/tool_attention.py:235`
...
### WR-01: WARNING - Memroos CI Does Not Run The Existing Lint Gate

**File:** `.github/workflows/ci.yml:27`

**Issue:** The Memroos job runs tests and build, but skips the app's `lint` script. Local lint currently reports React Compiler purity errors and TypeScript rule violations, so the workflow can pass code that violates the configured quality gate.

**Fix:** After clearing the current lint debt, add a CI step before build:
```yaml
- name: Run Memroos lint
  run: npm --prefix apps/memroos run lint
```
...
**File:** `apps/memroos/src/lib/paths.ts:17`
...
**File:** `apps/memroos/src/components/flow/react-flow-canvas.tsx:342`
...
**File:** `apps/memroos/src/lib/tool-attention.ts:46`
