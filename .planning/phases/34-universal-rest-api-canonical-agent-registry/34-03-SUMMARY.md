...
Added the Kitchen Agent Registry UI and migrated Kitchen Floor and Flow roster surfaces to canonical registered-agent data instead of hardcoded local/remote roster construction.

## Changes

- Added registry API client helpers and mutations in `apps/kitchen/src/lib/api-client.ts`:
  - `useRegisteredAgents()`
  - `registerAgent()`
  - `deregisterAgent()`
...
- Updated Kitchen Floor to source canonical registered agents from `/api/agents` and derive sections from registry fields.
...
- `npm --prefix apps/kitchen run test -- src/components/agents/__tests__/agent-registry-page.test.tsx src/components/flow/__tests__/registry-flow-roster.test.tsx` passed: 4 tests.
...
- GitNexus impact for `useAgents` was HIGH because Kitchen Floor, Flow, Voice, and Dispatch depend on it; the hook remains response-compatible while registry-specific helpers were added alongside it.
