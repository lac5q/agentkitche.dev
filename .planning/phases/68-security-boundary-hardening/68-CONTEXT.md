...
3. Long-input scanner bypass
   - `apps/memroos/src/lib/content-scanner.ts` allows content over its scanner limit without scanning.
   - Payloads over 4096 characters must be chunk-scanned or rejected fail-closed.

4. Partial Iris prompt-injection coverage
   - `apps/memroos/src/lib/iris-scanner.ts` currently uses regex rules for obvious instruction override, system prompt exfiltration, and tool-policy bypass patterns.
...
5. Capability compatibility default
   - `apps/memroos/src/lib/security-policy.ts` uses `allowLegacyWhenUndeclared()` for dispatch, A2A send, and memory write paths.
...
7. A2A private-network default
   - `apps/memroos/src/lib/a2a/config.ts` defaults remote-card private-network allowance permissively.
