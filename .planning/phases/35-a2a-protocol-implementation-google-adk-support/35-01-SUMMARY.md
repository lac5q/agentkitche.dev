...
provides:
  ...
  - Kitchen's public A2A agent card at the canonical and compatibility well-known paths
...
patterns-established:
  - "A2A foundation modules live under `apps/kitchen/src/lib/a2a` and isolate protocol/config/card concerns from existing dispatch cards."
  - "Compatibility well-known routes wrap the canonical card and mark `extensions.kitchen.compatibilityAlias` instead of becoming a second source of truth."
...
# Phase 35 Plan 01: A2A Foundation, Config, And Kitchen Agent Card Summary

**Config-derived A2A profile support with Kitchen's secure public agent card at canonical and compatibility well-known paths**
...
- Added Kitchen's own A2A agent card with streaming/history capabilities, safe skills, bearer auth declaration, and no leaked secrets or local paths.
...
3. **Task C: Build Kitchen's A2A agent card and well-known routes** - `809dd72` (feat)

**Plan metadata:** this summary commit

## Files Created/Modified

- `apps/kitchen/src/lib/a2a/config.ts` - A2A operating profile config, env overrides, URL normalization, and ADK fixture default.
- `apps/kitchen/src/lib/a2a/types.ts` - A2A 1.0 constants plus task, message, event, skill, security, and agent-card interfaces.
- `apps/kitchen/src/lib/a2a/errors.ts` - A2A error class and HTTP response mapper.
- `apps/kitchen/src/lib/a2a/agent-card.ts` - Kitchen public A2A card builder with bearer auth and safe public metadata.
- `apps/kitchen/src/lib/a2a/__tests__/agent-card.test.ts` - Contract tests for config, card, security leak prevention, and well-known routes.
- `apps/kitchen/src/app/.well-known/agent-card.json/route.ts` - Canonical public Kitchen A2A agent-card route.
- `apps/kitchen/src/app/.well-known/agent.json/route.ts` - Compatibility alias route that marks `compatibilityAlias: true`.
...
- **Found during:** Task C (Kitchen agent card implementation)
...
- **Files modified:** `apps/kitchen/src/lib/a2a/agent-card.ts`
- **Verification:** `npm --prefix apps/kitchen run test -- src/lib/a2a/__tests__/agent-card.test.ts`
...
- `npm --prefix apps/kitchen run typecheck` is listed in the plan but no `typecheck` script exists in `apps/kitchen/package.json`. Used `npm --prefix apps/kitchen run build` as the TypeScript verification equivalent; it completed successfully.
- Build emitted the known pre-existing Turbopack NFT warning through `/api/apo`, already documented outside this plan.
- Existing Vitest warning remains in `apps/kitchen/src/app/api/agents/__tests__/card.test.ts` for nested `vi.mock`; tests still pass and this warning predates Phase 35 execution.

## Verification

- `npm --prefix apps/kitchen run test -- src/lib/a2a/__tests__/agent-card.test.ts` - passed, 7 tests.
