...
key-decisions:
  ...
  - "The Google ADK proof is optional example infrastructure under `examples/`, not a Kitchen startup dependency."
...
**Kitchen can now delegate to registered A2A agents, prove the Google ADK path with an optional fixture, and surface A2A/ADK agents safely from the canonical roster.**
...
- `apps/kitchen/src/lib/a2a/client.ts` - Outbound A2A HTTP client, task lookup/cancel helpers, timeout handling, response truncation, and URL redaction.
- `apps/kitchen/src/lib/a2a/__tests__/client.test.ts` - Client contract tests for endpoint URL, auth env key, timeout, response truncation, and trace IDs.
- `apps/kitchen/src/lib/dispatch/a2a-adapter.ts` - Dispatch adapter that sends A2A tasks and mirrors recognized remote task state into Kitchen task storage when present.
- `apps/kitchen/src/lib/dispatch/__tests__/a2a-adapter.test.ts` - Adapter selection and remote completion mirroring tests.
- `apps/kitchen/src/lib/dispatch/adapter-factory.ts` - Selects A2A adapter when `agent.protocol === "a2a"`.
- `apps/kitchen/src/lib/dispatch/types.ts` - Allows adapters to receive the selected remote agent config.
- `apps/kitchen/src/lib/agent-registry.ts` - Carries protocol/metadata into remote configs and normalizes optional location for build safety.
- `apps/kitchen/src/types/index.ts` - Extends `RemoteAgentConfig` with optional protocol and metadata.
- `examples/adk-a2a-agent/*` - Optional ADK fixture and instructions.
- `apps/kitchen/src/lib/api-client.ts` - Adds `registerA2aAgentCard()` and mutation hook.
- `apps/kitchen/src/components/agents/*` - Adds A2A registration mode and safe A2A/ADK metadata display.
- `apps/kitchen/src/app/flow/page.tsx` and `apps/kitchen/src/components/flow/*` - Carries registry metadata into Flow and node details.
...
- Outbound credentials are intentionally indirect: registry metadata may name an environment variable, but Kitchen never stores or renders the secret value in UI.
- The ADK proof remains optional and operator-run. It demonstrates the install/run/card-registration path without adding ADK as a Kitchen runtime dependency.
...
- **Issue:** The plan's repo-root test paths produce `No test files found` when passed through `npm --prefix apps/kitchen` because Vitest runs from the package directory.
...
- **Files modified:** `apps/kitchen/src/lib/agent-registry.ts`
- **Verification:** `npm --prefix apps/kitchen run build` passed after the fix.
...
- `npm --prefix apps/kitchen run test -- src/lib/a2a/__tests__/client.test.ts src/lib/dispatch/__tests__/a2a-adapter.test.ts src/lib/dispatch/__tests__/adapter-factory.test.ts` - passed, 15 tests.
- `npm --prefix apps/kitchen run test -- src/lib/__tests__/agent-registry.test.ts src/lib/a2a/__tests__/card-ingestion.test.ts src/lib/a2a/__tests__/client.test.ts src/lib/dispatch/__tests__/a2a-adapter.test.ts src/lib/dispatch/__tests__/adapter-factory.test.ts` - passed, 27 tests.
...
- `npm --prefix apps/kitchen run test -- src/components/agents/__tests__/agent-registry-page.test.tsx` - passed, 4 tests.
- `npm --prefix apps/kitchen run test -- src/components/flow/__tests__/registry-flow-roster.test.tsx` - passed, 4 tests.
- `npm --prefix apps/kitchen run test -- src/components/agents/__tests__/agent-registry-page.test.tsx src/components/flow/__tests__/registry-flow-roster.test.tsx` - passed, 8 tests.
- `npm --prefix apps/kitchen run test -- src/lib/__tests__/agent-registry.test.ts src/lib/a2a/__tests__/client.test.ts src/lib/dispatch/__tests__/a2a-adapter.test.ts src/components/agents/__tests__/agent-registry-page.test.tsx src/components/flow/__tests__/registry-flow-roster.test.tsx` - passed, 21 tests.
- `npm --prefix apps/kitchen run test -- --run` - passed, 61 files and 414 tests.
- `npm --prefix apps/kitchen run lint` - passed with 12 pre-existing warnings.
- `npm --prefix apps/kitchen run build` - passed with known pre-existing Turbopack NFT warnings.
...
- Register the fixture card through Kitchen's protected `/api/a2a/agents/register` endpoint using the README curl example.

## Next Phase Readiness

Phase 35 is implementation-complete. Kitchen now has the A2A transport layer, canonical A2A registration, durable task lifecycle, outbound delegation, ADK proof fixture, and Registry/Flow surfacing needed for Phase 36 LangGraph orchestration to sit on top of A2A without owning transport concerns.
