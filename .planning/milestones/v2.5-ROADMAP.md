...
**Success Criteria** (what must be TRUE):
  ...
  2. The Memroos UI operator approval queue shows pending proposals with diff, forecast W-delta, and approve/reject controls; decisions are persisted
...
- [ ] **Phase 63**: Rename + Team Auth — Memroos → Memroos rename, RBAC (admin/operator/reviewer), multi-user JWT auth, team invitation
