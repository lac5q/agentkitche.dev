      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Explicit parent/child links use <span className="text-amber-400">masterId</span>. Runtime groups show which harness family each agent currently belongs to.
          </p>
        </div>
        <div className="flex gap-2 text-xs">
          <span className="rounded-full border border-sky-700/70 bg-sky-500/10 px-2.5 py-1 text-sky-300">
            {explicitHarnesses.length} explicit harnesses
          </span>
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          {runtimeGroups.map(([label, groupedAgents]) => (
            <div key={label} className="rounded-xl border border-slate-800 bg-slate-950/50 p-3">
              <p className="text-xs uppercase tracking-[0.16em] text-slate-500">runtime harness</p>
