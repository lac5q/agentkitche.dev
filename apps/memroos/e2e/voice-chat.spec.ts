...
test.describe("Voice & Chat panel — Flow page", () => {
  ...
  test("Voice & Chat panel is visible with agent selector", async ({ page }) => {
    ...
    // Agent dropdown exists with default "Memroos Floor"
    const select = page.locator("select").filter({ hasText: "Memroos Floor" });
  ...
  test("Chat tab is active by default and shows placeholder", async ({ page }) => {
    ...
    await expect(
      page.getByPlaceholder(/Message Memroos Floor/)
    ).toBeVisible();
  });
  ...
  test("Chat tab: send a message and receive a streaming response", async ({ page }) => {
    ...
    const input = page.getByPlaceholder(/Message Memroos Floor/);
    ...
    await expect(
      page.locator(".text-xs.rounded-lg").filter({ hasText: "Memroos Floor" }).last()
...
test.describe("Audit log panel — Memroos Floor", () => {
  test("Audit Log panel renders on Memroos Floor page", async ({ page }) => {
