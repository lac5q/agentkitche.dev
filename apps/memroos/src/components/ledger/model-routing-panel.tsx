  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <div>
          <h3 className="text-lg font-semibold text-slate-200">Model Routing</h3>
          <p className="mt-1 text-xs text-slate-500">Telemetry, recommendations, and eval coverage</p>
        </div>
        <div className="ml-auto flex flex-wrap gap-2">
          {TASK_TYPES.map((item) => (
            <button
              ...
              className={`border px-2.5 py-1 text-xs ${taskType === item ? "border-amber-500 text-amber-300" : "border-slate-700 text-slate-400"}`}
            >
              {item}
            </button>
          ))}
          {STRATEGIES.map((item) => (
            <button
              ...
              className={`border px-2.5 py-1 text-xs ${strategy === item ? "border-sky-500 text-sky-300" : "border-slate-700 text-slate-400"}`}
            >
              {item}
            </button>
          ))}
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="border border-slate-800 bg-slate-950/30 p-3">
          ...
          <p className="mt-1 text-2xl font-semibold text-slate-100">{summary?.totalRuns ?? 0}</p>
        </div>
        <div className="border border-slate-800 bg-slate-950/30 p-3">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="border border-slate-800 bg-slate-950/30 p-3">
          <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">Recommendations</p>
          {recommendations.isLoading ? (
            <p className="py-5 text-center text-sm text-slate-500">Loading recommendations...</p>
          ) : (
            <div className="space-y-2">
              {recs.map((rec) => (
                <div key={`${rec.provider}:${rec.model}`} className="grid gap-2 border-b border-slate-800/70 pb-2 last:border-0 last:pb-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-medium text-slate-200">{rec.model}</span>
                    <span className="rounded border border-slate-700 bg-slate-800/60 px-1.5 py-0.5 text-xs text-slate-400">
                      {rec.provider}
                    </span>
        ...
        <div className="border border-slate-800 bg-slate-950/30 p-3">
          <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">Eval Dimensions</p>
          <div className="space-y-2">
            {(evals.data?.dimensions ?? []).map((dimension) => (
              <div key={dimension.id} className="border-b border-slate-800/70 pb-2 last:border-0 last:pb-0">
                <p className="text-sm font-medium text-slate-200">{dimension.label}</p>
                <p className="mt-1 text-xs leading-5 text-slate-500">{dimension.rubric}</p>
              </div>
            ))}
          </div>
        </div>
