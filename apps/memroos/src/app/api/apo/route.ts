...
async function parseProposal(
  ...
  try {
    ...
    const tracking = await readProposalTracking(filePath, status, fileStat);
...
async function readProposalTracking(
  filePath: string,
  status: ApoProposalStatus,
  fileStat: Awaited<ReturnType<typeof stat>>
): Promise<ApoProposal["tracking"]> {
  ...
  const workItem = await readFile(`${filePath}.json`, "utf-8")
    .then((content) => JSON.parse(content) as ApprovalWorkItem)
    .catch((error: unknown) => {
  ...
  if (status === "approved") {
    const approvedAt = workItem?.approvedAt ?? timestamp;
    const executorCli = workItem?.executorCli ?? DEFAULT_EXECUTOR_CLI;
    return {
      phase: "queued",
      label: "Queued for worker",
      description: `Approved and waiting for ${executorCli} to implement it.`,
      timestamp: approvedAt,
      approvedAt,
      targetPath: workItem?.targetPath,
      targetKind: workItem?.targetKind,
      executorCli,
    };
  }

  const implementedAt = workItem?.implementedAt ?? workItem?.archivedAt ?? timestamp;
  return {
    phase: "implemented",
    label: workItem?.applied === false ? "Archived" : "Implemented",
    description:
      workItem?.applied === false
        ? "Worker processed this approval; the target already contained the requested constraint."
        : "Worker applied this approval and archived it for audit history.",
    timestamp: implementedAt,
    approvedAt: workItem?.approvedAt,
    implementedAt,
    archivedAt: workItem?.archivedAt ?? implementedAt,
    targetPath: workItem?.targetPath,
    targetKind: workItem?.targetKind,
    executorCli: workItem?.executorCli,
    applied: workItem?.applied,
  };
}
...
async function queueApprovedProposal(filename: string, executorCli = DEFAULT_EXECUTOR_CLI) {
  ...
  return {
    ...
    body: {
      ...
      queued: true,
      approvedAt,
      workerCommand: `npm --prefix apps/memroos run apo:worker -- --executor ${executorCli}`,
    },
  };
}
...
async function applyApprovedProposal(filename: string, executorCliOverride?: string) {
  ...
  const applied = !target.content.includes(constraint);
  if (applied) {
    const approvalBlock = [
      "",
  ...
  const archivedWorkItem: ApprovalWorkItem = {
    proposalId: identity.filename,
    skillId: identity.skillId,
    targetPath: target.path,
    targetKind: workItem?.targetKind ?? target.kind,
    executorCli,
    status: "implemented",
    approvedAt: workItem?.approvedAt ?? implementedAt,
    implementedAt,
    archivedAt: implementedAt,
    applied,
  };
  ...
  await writeFile(
    workItemPath(identity.filename, ARCHIVED_PATH),
    JSON.stringify(archivedWorkItem, null, 2),
    "utf-8"
  );
  await unlink(workItemPath(identity.filename)).catch((error: unknown) => {
