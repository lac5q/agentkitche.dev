...
export function getA2aConfig(): A2aConfig {
  ...
  const profile = parseProfile(process.env.MEMROOS_A2A_PROFILE);

  return {
    profile,
    publicBaseUrl,
    endpointBaseUrl: normalizeBaseUrl(
      process.env.MEMROOS_A2A_ENDPOINT_BASE_URL,
      publicBaseUrl
    ),
    canonicalCardPath: A2A_CANONICAL_AGENT_CARD_PATH,
    compatCardPath: A2A_COMPAT_AGENT_CARD_PATH,
    remoteCardTimeoutMs: parseTimeout(process.env.MEMROOS_A2A_REMOTE_CARD_TIMEOUT_MS),
    allowPrivateNetworkCards: parseBoolean(
      process.env.MEMROOS_A2A_ALLOW_PRIVATE_NETWORK_CARDS,
      profile === "local-dev" || profile === "private-network"
    ),
    adkFixtureCardUrl:
      process.env.MEMROOS_A2A_ADK_FIXTURE_CARD_URL?.trim() || DEFAULT_ADK_FIXTURE_CARD_URL,
  };
}
