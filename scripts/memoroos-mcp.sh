#!/usr/bin/env bash
# Launch Memoroos' knowledge/tool-attention MCP facade.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat >&2 <<'HELP'
Usage: scripts/memoroos-mcp.sh [--stdio|--http|--sse] [options]
...
Examples:
  ...
  scripts/memoroos-mcp.sh --http --host 0.0.0.0 --port 8765
HELP
      exit 0
      ;;
    *)
